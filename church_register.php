...
//$type=mysqli_real_escape_string($db->dbcon(), $_POST['type']);
$type=mysqli_real_escape_string($db->dbcon(), 1);
$linkname=mysqli_real_escape_string($db->dbcon(), $_POST['linkname']);
$status=mysqli_real_escape_string($db->dbcon(), 0);

$city=mysqli_real_escape_string($db->dbcon(), $_POST['city']);
$state=mysqli_real_escape_string($db->dbcon(), $_POST['state']);
$country=mysqli_real_escape_string($db->dbcon(), $_POST['country']);

$address=new stdClass;
$address->city=$city;
$address->state=$state;
$address->country=$country;
$leader=mysqli_real_escape_string($db->dbcon(), $_POST['leader']);
$memberapproval=mysqli_real_escape_string($db->dbcon(), $_POST['memberapproval']);
$denomination=mysqli_real_escape_string($db->dbcon(), $_POST['denomination']);
$phone=mysqli_real_escape_string($db->dbcon(), $_POST['phone']);
$tv=mysqli_real_escape_string($db->dbcon(), $_POST['tv']);
$address= json_encode($address);
$address=mysqli_real_escape_string($db->dbcon(), $address);
if($count < 1)
{

		       $concol = '`name`,`email`,`password`,`type`,`linkname`,`status`,`created_at`,`updated_at`';
			   $value = '"'.$name.'","'.$emailadd.'","'.$password.'","'.$type.'","'.$linkname.'","'.$status.'","'.$time.'","'.$time.'"';
			   $data=$db->SaveData($table,$concol,$value);
			   $last_saved_id = mysqli_insert_id($db->dbcon());
			  
			   if($data!=false){		  
			   
			   $concolinfo = '`userid`,`leader`,`memberapproval`,`contactemail`,`denomination`,`address`,`phone`,`tv`,`created_at`,`updated_at`';
			   $infovalue = '"'.$last_saved_id.'","'.$leader.'","'.$memberapproval.'","'.$emailadd.'","'.$denomination.'","'.$address.'","'.$phone.'","'.$tv.'","'.$time.'","'.$time.'"';
			   $infodata=$db->SaveData($churchinfotable,$concolinfo,$infovalue);
