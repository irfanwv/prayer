...
$device_type=mysqli_real_escape_string($db->dbcon(), $_POST['device_type']);
$device_token=mysqli_real_escape_string($db->dbcon(), $_POST['device_token']);
...
if($count < 1)
{
		...
		//       $concol = '`name`,`email`,`password`,`type`,`status`,`profilepic`,`created_at`,`updated_at`';
		//	 $value = '"'.$name.'","'.$emailadd.'","'.$password.'","'.$type.'","'.$status.'","'.$image.'","'.$time.'","'.$time.'"';
		        $concol = '`name`,`email`,`password`,`type`,`status`,`created_at`,`updated_at`, `device_type`, `device_token`';
			$value = '"'.$name.'","'.$emailadd.'","'.$password.'","'.$type.'","'.$status.'","'.$time.'","'.$time.'","'.$device_type.'","'.$device_token.'"';
			$data=$db->SaveData($table,$concol,$value);
			$last_saved_id = mysqli_insert_id($db->dbcon());
			$table1 = "userinfo" ;
			$concol1 = '`userid`, `country`,`created_at`';
			$value1 = "'".$last_saved_id."','".$country."','".$time."'";
			$data=$db->SaveData($table1,$concol1,$value1);
