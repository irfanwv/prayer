...
function sendmail($to,$subject,$message,$name)
    {
            ...
            try {
               ...
               // $mail->isSMTP();
                ...
                $mail->FromName = 'Mypray';
                $mail->addAddress($to, $name);     // Add a recipient
                $mail->addReplyTo("james01.matthew@gmail.com",$name);
...
if($count < 1)
{
	 $where_user = ' username="'.$_POST['username'].'" ';
	 $check_user = $db->selectcommand('*',$table,$where_user);
	 $count_user = mysqli_num_rows($check_user);
	 if($count_user < 1)
	 {
		  // $concol = '`name`,`email`,`password`, `device_id`, `address`, `city`,`state`,`zip`,`fav_manufacturer`';
		       $concol = '`name`, `username`,`email`,`password`, `type`,`linkname`,`status`, `profilepic`,`created_at`, `updated_at`';
			   $value = '"'.$_POST['name'].'","'.$_POST['username'].'","'.$_POST['email'].'","'.md5($_POST['password']).'","0","","'."0".'","'.$image.'","'.$time.'","'.$time.'"';
			   ...
			   $concolinfo = '`userid`, `gender`,`created_at`, `updated_at`';
			   $infovalue = '"'.$last_saved_id.'","'.$_POST['gender'].'","'.$time.'","'.$time.'"';
			   $infodata=$db->SaveData($infotable,$concolinfo,$infovalue);
			   
			   
			    $verification_code = $_POST['username'].mt_rand();
		        $username=$_POST['username'];
				$name=$_POST['name'];
				$email=$_POST['email'];
			   	//$varification_link="http://".getenv('HTTP_HOST')."/prayergrid/webservices/verification.php?id=$last_saved_id&veri_code=$verification_code&username=$username";
				$varification_link="http://".getenv('HTTP_HOST')."/prayergrid/verification.php?id=$last_saved_id&veri_code=$verification_code&username=$username";		
				$response["error"] = 0;
				$response["success"] = 1;
				...
				$user_to=$email;
				$body="<p>Name: $name</p><p>Username : $username</p> <p>Email: $email</p>";
			   ...
			   //$response["error"] = 0; $response["success"] = 1; $response["userid"] = $last_saved_id; $response["message"] = "Registration Successful";
			   
			   }else{$response["error"] = 1; $response["success"] = 0; $response["message"] = "Registration fail!";}
	}
	 else{$response["error"] = 1; $response["success"] = 0; $response["message"] = "Username already registered !";}
}
